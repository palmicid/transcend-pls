...
import TicTacToePlayerSlot, {
	PlayerRole,
	BOT_PLAYER_ID,
} from "./TicTacToePlayerSlot";
import { getBotMove } from "./TicTacToeBot";
...
> {
	...
	configureBot(
		role: "X" | "O" | null,
		difficulty: BotDifficulty | null = "Hard",
		delayMs: number = 500,
	): void {
	...
	private executeBotMove(): void {
		...
		const bestCell = getBotMove(
			...
			botRole,
			this.gameConfig.botDifficulty ?? "Hard",
		);
	...
	restoreState(data: TicTacToePersistedState): void {
		...
		if (
			data.current_turn &&
			(data.current_turn === "X" || data.current_turn === "O")
		) {
	...
	get Snapshot(): unknown {
		const isDraw =
			!this.gameState.winner && this.gameState.board.every((c) => c !== null);
