/**
 * @file app/game/tic-tac-toe/[roomId]/subscribe/route.ts
 * @description Server-Sent Events endpoint for Tic-Tac-Toe game subscriptions.
 *
 * Clients connect to receive real-time game updates.
 * For sending moves, use the submitTicTacToeMove server action.
 *
 * @example
 * ```ts
 * // Client-side
 * const eventSource = new EventSource('/game/tic-tac-toe/room-123/subscribe?userId=player-1');
 *
 * eventSource.addEventListener('snapshot', (event) => {
 *   const { snapshot } = JSON.parse(event.data);
 *   // snapshot contains: board, currentTurn, winner, players
 * });
 * ```
 */
...
/**
 * Handle SSE connection for Tic-Tac-Toe games.
 */
