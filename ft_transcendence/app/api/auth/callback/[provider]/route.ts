...
async function getUserInfo(provider: string, token_data: TokenResponse): Promise<UserInfo> {
  ...
  if (provider === 'google'){
    ...
    response = await fetchGithub('https://api.github.com/user', token_data);
  }
  if (!response.ok) {
...
async function getGithubEmail(token_data: TokenResponse): Promise<string> {
  const response = await fetchGithub('https://api.github.com/user/emails', token_data)
  if (!response.ok) {
    throw new Error("Failed to verify Token");
  }
...
async function fetchGithub(url: string, token_data: TokenResponse): Promise<Response>{
