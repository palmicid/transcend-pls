...
import { getBotMove } from "@/app/play/tic-tac-toe/lib/TicTacToeBot";
import type { PlayerRole } from "@/app/play/tic-tac-toe/lib/TicTacToePlayerSlot";
...
describe("TicTacToeBot - getBotMove", () => {
  describe("Immediate Win Detection", () => {
    it("should take winning move on row", () => {
      ...
      const move = getBotMove(board, "X", 9);
      expect(move).toBe(2);
    });
  ...
  describe("Difficulty Levels", () => {
    it("should return valid move for easy difficulty", () => {
      ...
      const move = getBotMove(board, "X", 1);
    ...
    it("should return valid move for medium difficulty", () => {
      const board: Board = ["X", null, null, null, "O", null, null, null, null];
      const move = getBotMove(board, "X", 3);
      expect(move).toBeGreaterThanOrEqual(0);
      expect(move).toBeLessThan(9);
      expect(board[move!]).toBeNull();
    });
  ...
  describe("Consistency and Determinism", () => {
    it("should return same move for same board state at max difficulty", () => {
      const board: Board = ["X", null, null, null, "O", null, null, null, null];
      const move1 = getBotMove(board, "X", 9);
      const move2 = getBotMove(board, "X", 9);
    ...
    it("should always return valid moves", () => {
      ...
      for (let i = 0; i < 50; i++) {
        const board: Board = Array(9).fill(null).map((_, idx) => {
