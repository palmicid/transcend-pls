...
import { BOT_PLAYER_ID } from "@/lib/game/tic-tac-toe/TicTacToePlayerSlot";
...
describe("Bot Integration Tests", () => {
	...
	describe("Bot Move Execution", () => {
		...
		it("should alternate turns correctly with bot", async () => {
			game.configureBot("O", "Hard", 50); // 50ms delay for test
			game.handlePlayerConnect("player-1");
			game.startGame();
	...
	describe("Bot State Persistence", () => {
		...
		it("should include bot config in snapshot", () => {
			game.configureBot("X", "Medium", 750);
			game.handlePlayerConnect("player-1");
			...
			expect(snapshot.bot.difficulty).toBe("Medium");
